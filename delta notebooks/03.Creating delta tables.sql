...
-- MAGIC %md
-- MAGIC Uso de apóstrofe no nome do schema.\
-- MAGIC Se não especifica o schema ele é colocado como "default".
...
-- MAGIC %md
-- MAGIC A presença do diretório "`_delta_log`" indica que trata-se de um delta lake ou um delta table.\
-- MAGIC Como não adicionamos nenhum dado ainda não temos parquet files 
...
-- MAGIC %md
-- MAGIC Examinando o arquivo json dentro de _delta_log, temos informações sobre a operação "CREATE TABLE" e sobre o schema criado.
...
-- MAGIC %md
-- MAGIC Necessário especificar o schema para não ser criado dentro de `"default"`
...
-- MAGIC %md
-- MAGIC Houve a geração de um parquet file com as informações dos dados coletados.
...
-- MAGIC %md
-- MAGIC Houve a geração de um arquivo json
